...
//------Arduino腳位設定------//
const int motorIn1=5;//定義ABS馬達接腳1為5
const int motorIn2=6;//定義ABS馬達接腳2為6
int Btn_ABS_ONOFF=7; //定義ABS開啟按鈕為7
int Btn_Stop=8;      //定義煞車按鈕為8
int VR_pin=A0;       //定義可變電阻輸入腳位為A0
const int motorIn11=11;//定義模擬轉速馬達接腳1為11
const int motorIn12=12;//定義模擬轉速馬達接腳2為12
...
void loop() {
  ...
  if ((currentTime1-time_1)==INTERVAL_MESSAGE2){
    ...
    count_step();
    analogWrite(motorIn1,0);
    analogWrite(motorIn11,analogRead(VR_pin));
